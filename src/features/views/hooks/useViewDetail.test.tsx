...
describe("useViewDetail", () => {
  ...
  describe("UUID validation", () => {
    it("marks an invalid UUID as invalid", () => {
      ...
      const { result } = renderHook(() => useViewDetail(INVALID_ID, OWNER, 0), { wrapper });
      expect(result.current.isViewIdValid).toBe(false);
    });

    it("marks a valid UUID as valid", () => {
      const { wrapper } = makeWrapper();
      const { result } = renderHook(() => useViewDetail(VALID_ID, OWNER, 0), { wrapper });
  ...
  describe("profiles", () => {
    ...
    it("shows pending characters immediately when adding new ones", async () => {
      ...
      mockPollOperation.mockImplementation(() => new Promise((res) => { resolvePoll = res; }));
      ...
      act(() => { result.current.saveCharacters([charA, charB]); });
      ...
      await act(async () => { resolvePoll({ id: "op-123", status: "COMPLETED" }); });
    ...
    it("keeps deleted characters visible with score null while the operation is in flight", async () => {
      ...
      act(() => { result.current.saveCharacters([charA]); });
